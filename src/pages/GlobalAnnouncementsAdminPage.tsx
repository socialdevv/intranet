...
import PlatformAnnouncementsManager from "@/components/admin/platform-announcements-manager";
import AppShell from "@/components/layout/app-shell";
import { useAuth } from "@/contexts/auth-context";
import { canAccessAdminPanel } from "@/lib/auth/authorization";
import { ROUTES } from "@/lib/routes";

export default function GlobalAnnouncementsAdminPage() {
  const { user } = useAuth();
  ...
  return (
    <AppShell
      ...
      searchPlaceholder="Szukaj po ogłoszeniach globalnych…"
    >
      <section className="mx-auto w-full max-w-7xl pb-8">
        <div className="rounded-4xl border border-[#dbe5f0] bg-[linear-gradient(135deg,#f8fbff_0%,#eef4fb_45%,#ffffff_100%)] px-6 py-8 shadow-[0_18px_60px_rgba(15,23,42,0.07)] dark:border-[#1e3a5f] dark:bg-[linear-gradient(135deg,#0f2340_0%,#102846_45%,#0f172a_100%)] sm:px-8 lg:px-10">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-[#475569] dark:text-[#cbd5e1]">
            Ten widok zarządza wyłącznie ogłoszeniami wyświetlanymi na globalnej stronie głównej. Treść pozostaje platformowa, nie projektowa, i korzysta z tego samego edytora rich text co istniejące przepływy komunikatów.
          </p>
        </div>
